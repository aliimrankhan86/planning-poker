...
|---|---|---|
| `src/App.js` | The entire app: CSS string, all components, all Firebase logic | 422 KB |
...
| `scripts/gen-ai-context.mjs` | Generates this file | 8 KB |
| `database.rules.json` | Firebase rules, with comments. **Source of truth.** | 18 KB |
| `database.rules.publish.json` | Generated. This is what gets pasted into the Firebase console | 13 KB |
...
Top-level nodes: `rooms`, `admins`, `analytics`, `users`, `licenses`, `history`, `$other`.
...
22 tests across AdminDashboard.test.js, App.test.js. They cover the two things that break
